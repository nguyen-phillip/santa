...
- (void)addBundleEvent:(SNTStoredEvent *)event reply:(void (^)(BOOL))reply {
  ...
  [self dispatchBlockOnSyncdQueue:^{
    [self.syncdConnection.remoteObjectProxy postBundleEventToSyncServer:event
      ...
      // related events synced using addEvents:fromBundle:.
...
- (void)addEvents:(NSArray<SNTStoredEvent *> *)events fromBundle:(BOOL)fromBundle {
  ...
  NSString *hash = fromBundle ? first.fileBundleHash : first.fileSHA256;
  if (![self backoffForPrimaryHash:hash]) return;
  [self dispatchBlockOnSyncdQueue:^{
    [self.syncdConnection.remoteObjectProxy postEventsToSyncServer:events fromBundle:fromBundle];
  }];
}
...
// The passed-in hash is fileBundleHash for a bundle event, or fileSHA256 for normal event.
